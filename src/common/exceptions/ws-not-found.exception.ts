...
export class WsNotFoundException<R extends Omit<WsClientExceptionInterface, "status"> = Omit<WsClientExceptionInterface, "status">> extends WsClientExceptionClass<R & Pick<WsClientExceptionInterface, "status">> {
    constructor(exception: R) {
        super({ ...exception, status: WsExceptionStatusEnum.NOT_FOUND });
    }
}